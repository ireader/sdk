...
struct stun_agent_handler_t
{
	...
	/// get pwd (realm/nonce if usr is null)
	/// @param[in/out] realm/nonce, if usr is null, return realm/nonce
	...
	int (*auth)(void* param, const char* usr, char realm[256], char nonce[256], char pwd[256], int *cred);
	...
	int (*onpermission)(void* param, stun_response_t* resp, const stun_request_t* req, const struct sockaddr_storage* peer);
	int (*onchannel)(void* param, stun_response_t* resp, const stun_request_t* req, const struct sockaddr_storage* peer, uint16_t channel);
...
typedef void (*turn_agent_ondata)(void* param, const void* data, int byte, int protocol, const struct sockaddr_storage* local, const struct sockaddr_storage* remote);
...
int turn_agent_create_permission(stun_request_t* req, const struct sockaddr_storage* peer);
/// @param[in] channel valid range: [0x4000, 0x7FFE]
int turn_agent_channel_bind(stun_request_t* req, const struct sockaddr_storage* peer, uint16_t channel);
/// Send data from client to turn server(and forward to peer)
int turn_agent_send(stun_request_t* req, const struct sockaddr_storage* peer, const void* data, int bytes);
...
void* stun_timer_start(int ms, void(*ontimer)(void* param, void* tid), void* param);
