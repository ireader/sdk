...
struct dht_message_handler_t
{
	...
	int (*pong)(void* param, int code, uint16_t transaction, const uint8_t id[20]);

	int (*find_node)(void* param, const uint8_t* transaction, uint32_t bytes, const uint8_t id[20], const uint8_t target[20]);
	int (*find_node_reply)(void* param, int code, uint16_t transaction, const uint8_t id[20], const struct node_t* nodes, uint32_t count);

	int (*get_peers)(void* param, const uint8_t* transaction, uint32_t bytes, const uint8_t id[20], const uint8_t info_hash[20]);
	int (*get_peers_reply)(void* param, int code, uint16_t transaction, const uint8_t id[20], const uint8_t* token, uint32_t bytes, const struct node_t* nodes, uint32_t count, const struct sockaddr_storage* peers, uint32_t peernum);
	...
	int (*announce_peer_reply)(void* param, int code, uint16_t transaction, const uint8_t id[20]);
};
