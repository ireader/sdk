...
#define DHT_PING_TID			'p'
#define DHT_FIND_NODE_TID		'f'
#define DHT_GET_PEERS_TID		'g'
#define DHT_ANNOUNCE_PEER_TID	'a'
#define DHT_VOTE_TID			'V'
#define DHT_GET_TID				'G'
#define DHT_PUT_TID				'P'
#define DHT_PUNCH_TID			'H'
...
static int dht_message_read_find_node_reply(const struct bvalue_t *r, const struct bvalue_t *id, struct dht_message_handler_t* handler, void* param);
static int dht_message_read_get_peers_reply(const struct bvalue_t *r, const struct bvalue_t *id, struct dht_message_handler_t* handler, void* param);
...
int dht_ping_write(uint8_t* buffer, uint32_t bytes, const uint8_t transaction[3], const uint8_t id[20])
{
	...
	n = snprintf((char*)buffer, bytes, "d1:t4:%c---1:y1:q1:q4:ping1:ad2:id20:", DHT_PING_TID);
	if (n < 0 || n + 2 + 20 >= (int)bytes)
		return -1;

	memcpy(buffer + 7, transaction, 3);
...
int dht_find_node_write(uint8_t* buffer, uint32_t bytes, const uint8_t transaction[3], const uint8_t id[20], const uint8_t target[20])
{
	...
	// find_node Query = {"t":"aa", "y":"q", "q":"find_node", "a": {"id":"abcdefghij0123456789", "target":"mnopqrstuvwxyz123456"}}
	n = snprintf((char*)buffer, bytes, "d1:t4:%c---1:y1:q1:q9:find_node1:ad2:id20:", DHT_FIND_NODE_TID);
...
int dht_find_node_reply_write(uint8_t* buffer, uint32_t bytes, const uint8_t* transaction, uint32_t tidbytes, const uint8_t id[20], const struct node_t* nodes, uint32_t count)
{
	...
	// Response = {"t":"aa", "y":"r", "r": {"id":"0123456789abcdefghij", "nodes": "def456..."}}
	n = snprintf((char*)buffer, bytes, "d1:t%u:", (unsigned int)tidbytes);
	if (n < 0 || n + tidbytes + 20 + len + 20 > bytes)
		return -1;
	memcpy(buffer + n, transaction, tidbytes);
	n += tidbytes;
...
int dht_get_peers_write(uint8_t* buffer, uint32_t bytes, const uint8_t transaction[3], const uint8_t id[20], const uint8_t info_hash[20])
{
	...
	n = snprintf((char*)buffer, bytes, "d1:t4:%c---1:y1:q1:q9:get_peers1:ad2:id20:", DHT_GET_PEERS_TID);
...
int dht_get_peers_reply_write(uint8_t* buffer, uint32_t bytes, const uint8_t* transaction, uint32_t tidbytes, const uint8_t id[20], const uint8_t* token, uint32_t token_bytes, const struct node_t* nodes, uint32_t count, const struct sockaddr_storage* peers, uint32_t peernum)
{
	...
	n = snprintf((char*)buffer, bytes, "d1:t%u:", (unsigned int)tidbytes);
	if (n < 0 || n + tidbytes + 20 + len + 20 + token_bytes + 16 + len2 + 16 > bytes)
		return -1;
	memcpy(buffer + n, transaction, tidbytes);
	n += tidbytes;
...
int dht_announce_peer_write(uint8_t* buffer, uint32_t bytes, const uint8_t transaction[3], const uint8_t id[20], const uint8_t info_hash[20], uint16_t port, const uint8_t* token, uint32_t token_bytes)
{
	...
	n = snprintf((char*)buffer, bytes, "d1:t4:%c---1:y1:q1:q13:announce_peer1:ad2:id20:", DHT_ANNOUNCE_PEER_TID);
...
int dht_message_read(const uint8_t* buffer, uint32_t bytes, struct dht_message_handler_t* handler, void* param)
{
	...
	else if ('r' == y->v.str.value[0])
	{
		...
		{
		case DHT_PING_TID:
			ret = handler->pong(param, 0, (uint8_t*)id->v.str.value);
			break;

		case DHT_FIND_NODE_TID:
			ret = dht_message_read_find_node_reply(r, id, handler, param);
			break;

		case DHT_GET_PEERS_TID:
			ret = dht_message_read_get_peers_reply(r, id, handler, param);
			break;

		case DHT_ANNOUNCE_PEER_TID:
			ret = handler->announce_peer_response(param, 0, (uint8_t*)id->v.str.value);
			break;
	...
	else if ('e' == y->v.str.value[0])
	{
		...
		if (!e || BT_LIST != e->type || e->v.list.count < 2)
		{
			bencode_free(&root);
			return -1;
		}
		...
		{
		case DHT_PING_TID:
			ret = handler->pong(param, (int)id->v.value, NULL);
			break;

		case DHT_FIND_NODE_TID:
			ret = handler->find_node_response(param, (int)id->v.value, NULL, NULL, 0);
			break;

		case DHT_GET_PEERS_TID:
			ret = handler->get_peers_response(param, (int)id->v.value, NULL, NULL, 0, NULL, 0, NULL, 0);
			break;
			
		case DHT_ANNOUNCE_PEER_TID:
			ret = handler->announce_peer_response(param, (int)id->v.value, NULL);
			break;
...
static int dht_message_read_find_node_reply(const struct bvalue_t *r, const struct bvalue_t *id, struct dht_message_handler_t* handler, void* param)
{
	...
	ret = handler->find_node_response(param, 0, (uint8_t*)id->v.str.value, items, n);
	free(items);
	return ret;
}
...
static int dht_message_read_get_peers_reply(const struct bvalue_t *r, const struct bvalue_t *id, struct dht_message_handler_t* handler, void* param)
{
	...
	if (!token || BT_STRING != token->type || token->v.str.bytes < 1
		|| ((!nodes || BT_STRING != nodes->type || nodes->v.str.bytes < 26) && (!nodes6 || BT_STRING != nodes6->type || nodes->v.str.bytes < 38) && (!values || BT_LIST != values->type)))
	{
		return -1;
	}
	...
	ret = handler->get_peers_response(param, 0, (uint8_t*)id->v.str.value, (uint8_t*)token->v.str.value, token->v.str.bytes, items, n, peers, m);
...
{
	uint8_t buffer[256];
	const uint8_t ping[] = "d1:t4:p0001:y1:q1:q4:ping1:ad2:id20:00000000000000000000ee";
	const uint8_t pong[] = "d1:t4:p0001:y1:r1:rd2:id20:99999999999999999999ee";
	const uint8_t find_node[] = "d1:t4:f0001:y1:q1:q9:find_node1:ad2:id20:000000000000000000006:target20:xxxxxxxxxxxxxxxxxxxxee";
	const uint8_t find_node_reply[] = "d1:t4:f0001:y1:r1:rd2:id20:999999999999999999995:nodes104:11111111111111111111111111222222222222222222222222223333333333333333333333333344444444444444444444444444ee";
	const uint8_t get_peers[] = "d1:t4:g0001:y1:q1:q9:get_peers1:ad2:id20:000000000000000000009:info_hash20:xxxxxxxxxxxxxxxxxxxxee";
	const uint8_t get_peers_reply[] = "d1:t4:g0001:y1:r1:rd2:id20:999999999999999999995:token6:secret5:nodes104:111111111111111111111111112222222222222222222222222233333333333333333333333333444444444444444444444444446:valuesl6:5555556:666666eee";
	const uint8_t announce_peer[] = "d1:t4:a0001:y1:q1:q13:announce_peer1:ad2:id20:000000000000000000009:info_hash20:xxxxxxxxxxxxxxxxxxxx5:token6:secret4:porti65535eee";
	const uint8_t announce_peer_reply[] = "d1:t4:a0001:y1:r1:rd2:id20:99999999999999999999ee";
	const struct node_t nodes[] = {
	...
	assert(sizeof(ping) == dht_ping_write(buffer, sizeof(buffer), (const uint8_t*)"000", (const uint8_t*)"00000000000000000000") + 1);
	assert(0 == memcmp(buffer, ping, sizeof(ping) - 1));
	assert(sizeof(pong) == dht_pong_write(buffer, sizeof(buffer), (const uint8_t*)"p000", 4, (const uint8_t*)"99999999999999999999") + 1);
	assert(0 == memcmp(buffer, pong, sizeof(pong) - 1));

	assert(sizeof(find_node) == dht_find_node_write(buffer, sizeof(buffer), (const uint8_t*)"000", (const uint8_t*)"00000000000000000000", (const uint8_t*)"xxxxxxxxxxxxxxxxxxxx") + 1);
	assert(0 == memcmp(buffer, find_node, sizeof(find_node) - 1));
	assert(sizeof(find_node_reply) == dht_find_node_reply_write(buffer, sizeof(buffer), (const uint8_t*)"f000", 4, (const uint8_t*)"99999999999999999999", nodes, sizeof(nodes)/sizeof(nodes[0])) + 1);
	assert(0 == memcmp(buffer, find_node_reply, sizeof(find_node_reply) - 1));

	assert(sizeof(get_peers) == dht_get_peers_write(buffer, sizeof(buffer), (const uint8_t*)"g000", (const uint8_t*)"00000000000000000000", (const uint8_t*)"xxxxxxxxxxxxxxxxxxxx") + 1);
	assert(0 == memcmp(buffer, get_peers, sizeof(get_peers) - 1));
	assert(sizeof(get_peers_reply) == dht_get_peers_reply_write(buffer, sizeof(buffer), (const uint8_t*)"g000", 4, (const uint8_t*)"99999999999999999999", (const uint8_t*)"secret", 6, nodes, sizeof(nodes) / sizeof(nodes[0]), peers, sizeof(peers) / sizeof(peers[0])) + 1);
	assert(0 == memcmp(buffer, get_peers_reply, sizeof(get_peers_reply) - 1));

	assert(sizeof(announce_peer) == dht_announce_peer_write(buffer, sizeof(buffer), (const uint8_t*)"000", (const uint8_t*)"00000000000000000000", (const uint8_t*)"xxxxxxxxxxxxxxxxxxxx", 65535, (const uint8_t*)"secret", 6) + 1);
	assert(0 == memcmp(buffer, announce_peer, sizeof(announce_peer) - 1));
	assert(sizeof(announce_peer_reply) == dht_announce_peer_reply_write(buffer, sizeof(buffer), (const uint8_t*)"a000", 4, (const uint8_t*)"99999999999999999999") + 1);
