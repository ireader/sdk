...
static int tls_create(tlskey_t* key)
{
#if defined(OS_WINDOWS)
	*key = TlsAlloc();
...
///@return 0-ok, other-error
static int tls_destroy(tlskey_t key)
{
#if defined(OS_WINDOWS)
...
///@return 0-ok, other-error
static int tls_setvalue(tlskey_t key, void* value)
{
#if defined(OS_WINDOWS)
...
static void* tls_getvalue(tlskey_t key)
{
#if defined(OS_WINDOWS)
